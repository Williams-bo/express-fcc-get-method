<template>
    <section class="mx-3 bg-[#F4F2FE] rounded-b-[32px] pb-16">
        <div
            class=" flex flex-col space-y-10 items-start lg:items-center p-5 min-h-[85vh] md:justify-between md:space-y-0 md:space-x-10  md:p-0 md:flex-row md:px-6 lg:px-24 ">
            <div
                class="flex flex-col text-left w-fit justify-center mt-10 space-y-5 transition-all max-w-[38rem] md:text-left md:min-h-fit">

                <h1
                    class=" font-medium text-fadePurple text-4xl md:justify-start md:text-left lg:text-6xl">
                    Welcome to Ire
                </h1>
                ...
                <div class="flex space-x-3 justify-start">
                    <Button text="Book inspection" @click="setForm('Book inspection')" design="btn-xl bg-veryDarkBlue rounded-full" />
                    <Button text="Request a call" @click="setForm('Request a call')" design="btn-xl bg-white text-veryDarkBlue rounded-full" />
                </div>

            </div>

            <div class=" flex justify-start items-start min-h-[40vh] max-h-fit w-full md:w-[35rem] md:block md:h-fit">
